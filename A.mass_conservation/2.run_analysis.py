...
relabel_Pangu6 = (
    True  # whether to relabel "Pangu6" to "Pangu24" and only use every fourth lead time
)
...
for model in models:
    ...
    ylab = (
        ...
        if plot_var == "sp"
            ...
            "Lat-weighted SSP (hPa)" if plot_var == "ssp" else "Lat-weighted MSLP (hPa)"
        )
    )
    linewidth = 2
    fontsize = 24
    ...
    qual_colors = (
        cmap(np.linspace(0, 1, n_ics))
        if cmap is not None
        else [cmap_str.split(":")[1]] * n_ics
    )
    ...
    for i, ic in enumerate(ic_dates):
        ...
        ax.plot(
            ...
            alpha=0.8,
            linewidth=linewidth,
            label=f"Forecast init   {ic.strftime('%Y-%m-%d %Hz')}",
    ...
    if model == "Pangu24":  # only 24 hour timestep
        ax.set_xticks(
            lead_times[::day_interval_x_ticks],
            (lead_times[::day_interval_x_ticks] // (24)),
            fontsize=smallsize,
        )
    else:
        ax.set_xticks(
...
if cmap_str.startswith("single:"):
    cmap = colormaps.get_cmap(
        "Dark2"
    )  # override single color if multiple models are being plotted
else:
...
if plot_base_fields:
    ...
    titles = [
        f"{model}: MSLP init {ic_dates[0].strftime('%d-%m-%Y %Hz')} @ {(t/4):.2f} days lead time"
        for t in np.arange(0, n_timesteps + 1)
    ]
