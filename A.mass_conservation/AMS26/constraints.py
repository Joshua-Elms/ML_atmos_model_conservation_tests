...
# read configuration
config_path = Path(
    "/N/slate/jmelms/projects/ML_atmos_model_conservation_tests/A.mass_conservation/data/SFNO_FCNv2_ACE2/config.yaml"
)
...
title = "Mass Conservation in Various ML Models"  # generic title
save_title = "constraints.png"
ylab = rf"Global {display_var} (hPa)" if plot_var == "sp" else "Global MSLP (hPa)"
n_ics = len(ic_dates)
n_models = len(models)
...
if standardized_ylims:
    ...
    try:
        val_range = (
            math.floor(ds[f"MEAN_{plot_var}"].sel(model=models).min()),
            math.ceil(ds[f"MEAN_{plot_var}"].sel(model=models).max()),
        )
